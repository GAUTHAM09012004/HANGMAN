...
Display = pygame.display.set_mode((1100,680),0,32)
Easy = ["STAR","BELL","PICNIC", "PIE","HAT","HEART","FLAG"]
...
def StartScreen():
    Display.blit(pygame.font.Font("freesansbold.ttf", 45).render("HANGMAN", True, BLACK), (20, 20))
    Display.blit(Font.render("Level Difficulty", True, BLUE), (200, 150))
    ...
    Display.blit(Font.render("INSTRUCTIONS:", True, RED), (570, 50))
    Display.blit(Font2.render("1. The player tries to guess what the word is.", True, BLACK), (570, 95))
    Display.blit(Font2.render("  one letter at a time.", True, BLACK), (570, 125))
    Display.blit(Font2.render("2. There are number of dashes equivalent to the", True, BLACK), (570, 160))
    Display.blit(Font2.render("   number of letters in the word.", True, BLACK), (570, 190))
    Display.blit(Font2.render("3. If a player suggests the letter which ", True, BLACK), (570, 225))
    Display.blit(Font2.render("   occurs in the word,the game fills the blank ", True, BLACK), (570, 255))
    Display.blit(Font2.render("   with that letter in the right places.", True, BLACK), (570, 285))
    Display.blit(Font2.render("4. As the game progress,a segment of the", True, BLACK), (570, 320))
    Display.blit(Font2.render("   gallows and of the victim is added for", True, BLACK), (570, 350))
    Display.blit(Font2.render("   every suggested letter not in the word.", True, BLACK), (570, 380))
    Display.blit(Font2.render("5. The game ends when the character in the", True, BLACK), (570, 415))
    Display.blit(Font2.render(" word is still not guessed. ", True, BLACK), (570, 445))
    Display.blit(Font2.render("                    Developed by : S Gautham  ", True, BLACK), (280, 630))
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == QUIT:
                ...
                pygame.draw.rect(Display, WHITE, (260, 50, 200, 100))  # hide invalid
                UserInput = event.key
                # print(chr(event.key))
                # This is used to check if it changes the ASCII int value to the char
